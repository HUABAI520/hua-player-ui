...
const Player: React.FC = () => {
  ...
  const [collectionsList, setCollectionsList] = useState<API.Collections[]>([]);
  const [checkedCollections, setCheckedCollections] = useState<number[]>([]);
  ...
  useEffect(() => {
    ...
    if (artInstance.current) {
      ...
      artInstance.current.on('restart', () => {
        // 1. 当重新播放视频时，如果是当前动漫获取到最近播放视频且自动切换到时，更新播放进度
        artInstance?.current?.hotkey.add('75', hotkeyEvent);
        if (animeMsg?.seek && animeMsg?.lookVideoId === fileVideo?.id && animeMsg?.seek > 0) {
          if (artInstance.current) artInstance.current.seek = Number(animeMsg?.seek || 0);
        }
        // 2. 当重新播放视频时，如果是手动切换的因为后端保存了观看了的所有视频的记录，所以这里需要更新播放进度
        else if (
          animeMsg?.lookVideoId !== fileVideo?.id &&
          fileVideo?.seek &&
          fileVideo?.seek > 0
        ) {
          if (artInstance.current) artInstance.current.seek = Number(fileVideo?.seek || 0);
        }
      });
  ...
  const fetchCollections = async () => {
    setLoading(true);
    try {
      // 获取用户所有收藏夹 todo 这个两个请求可以合并到一个请求 获取所有收藏夹 联表查询该动漫和该用户收藏了的收藏夹（状态区分
      const res = await collections({ current: 1, pageSize: 100 });
      if (res?.records) {
        setCollectionsList(res.records);

        // 获取当前动漫的收藏状态
        const isRes = await collectionsAnimeIs({ aid: animeMsg?.id || 0 });
        setCheckedCollections(isRes || []);
      }
    } catch (e) {
  ...
  // 处理收藏/取消收藏
  const handleCollect = async (cid: number, checked: boolean) => {
    try {
      if (checked) {
        const res = await collectionsAnimeAdd({ cid, aid: animeMsg?.id || 0 });
        if (res) {
        ...
        const res = await collectionsAnimeRemove({ aId: animeMsg?.id || 0, cId: cid });
        if (res) {
      ...
      const newCheckedCollections = checked
        ? [...checkedCollections, cid]
        : checkedCollections.filter((id) => id !== cid);
      // 更新本地选中状态
      ...
      if (animeMsg) {
        animeMsg.isCollect =
          animeMsg.isCollect === 0 && newCheckedCollections.length > 0
            ? 1
            : animeMsg.isCollect === 1 && newCheckedCollections.length === 0
            ? 0
            : animeMsg.isCollect;
      }
    } catch (e) {
        ...
        >
          <Flex vertical justify={'start'} align={'start'}>
            <Flex gap={'12px'} style={{ width: '100%' }}>
              ...
              >
                <Flex gap={24} justify={'space-between'} style={{ width: '50%' }} align={'end'}>
                  <Typography.Title level={3}>{animeMsg?.name}</Typography.Title>
                  <Tooltip color={token.colorPrimary} title={'收藏'}>
                    <Rate
                      count={1} // 只显示一个星星
                      value={animeMsg?.isCollect ? 1 : 0} // 根据 isCollect 设置星星是否点亮
                      character={<StarFilled />} // 使用实心星星图标
                      style={{
                        color: animeMsg?.isCollect ? '#ffc107' : '#ccc',
                        fontSize: 24,
                        marginBottom: 12,
                      }} // 根据 isCollect 设置颜色
                      onChange={(value) => {
                        setCollectionModalVisible(true);
                        fetchCollections(); // 打开弹窗时获取收藏夹数据
                      }}
                    />
                  </Tooltip>
                </Flex>
          ...
          <List
            ...
            renderItem={(item) => (
              <List.Item
                actions={[
                  <Checkbox
                    checked={checkedCollections.includes(item.id!)}
                    onChange={(e) => handleCollect(item.id!, e.target.checked)}
                  />,
                ]}
              >
                <List.Item.Meta
                  title={item.title}
                  description={`包含 ${item.count} 个内容`}
                  avatar={
                    item.image ? (
                      <Image src={item.image} width={48} height={48} style={{ borderRadius: 4 }} />
                    ) : null
                  }
                />
              </List.Item>
